...
`main` 브랜치는 별도 백그라운드 프로세스 없이 페이지에 접속할 때마다 가동계획의 경과시간을 계산해 생산량과 완제품 재고를 반영합니다. 설비 성과 화면을 열어 둔 동안에는 10초마다 자동 갱신합니다. Ubuntu `systemd` Worker 방식은 `codex/ubuntu-worker-version-20260805` 브랜치에 보존되어 있습니다.
